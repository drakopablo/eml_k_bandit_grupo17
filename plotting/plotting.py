...
Based Luis Daniel Hernández Molinero (ldaniel@um.es) previous work
Date: 2026/02/04
...
def plot_optimal_selections(steps: int, optimal_selections: np.ndarray, algorithms: List[Algorithm]):
    ...
    for idx, algo in enumerate(algorithms):
        label = get_algorithm_label(algo)
        plt.plot(range(steps), optimal_selections[idx], label=label, linewidth=2)  # Multiplica por 100
    plt.xlabel('Steps', fontsize=14)
    plt.ylabel('% Optimal action', fontsize=14)  # Etiqueta corregida
